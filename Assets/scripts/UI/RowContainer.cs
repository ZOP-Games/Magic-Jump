    ...
    public class RowContainer : Container
    {
        protected override void BuildLayout()
        {
            ...
            for (var i = 0; i < elements.Length; i++)
            {
                ...
                pos.x = totalX+delta.x / 2+(i+1)*padding;
                totalX += delta.x;
                pos.y = delta.y / 2;
        ...
        private void Start()
        {
            elements = GetComponentsInChildren<UIComponent>().Where(c =>  c.gameObject != gameObject).Select(c => c.GetComponent<RectTransform>()).ToArray();
            BuildLayout();
        }
    }
}